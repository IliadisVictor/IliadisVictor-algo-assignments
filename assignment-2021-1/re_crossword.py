...
def RegularExpressionsDictionary(filename,RegularExprDict):
    with open(filename, 'r') as csvfile:
        reader = csv.reader(csvfile)
        for row in reader:
            RegularExprDict.update({row[0]: -1})
...
def SelectWordToFill(CrosswordDict):
    ...
    if SelectedKey ==-1:
        for i in range (0,len(CrosswordDict)):
            if i not in RegularExpressions.values():
                return i
    return SelectedKey


def SolveCrossword(CrosswordsInput):
    ...
    for RegularExpression in reversed(RegularExpressions):
        if RegularExpressions[RegularExpression] == -1:
            ...
            for CandidateWord in TestWords:
                if ValidateWord(CandidateWord,EmptyWordKey,CrosswordsInput):
                    ...
                    RegularExpressions[RegularExpression]= EmptyWordKey

                    if SolveCrossword(CrosswordsInput):
                        return True
                ...
                UpdateCrossword(EmptyWordKey,CrosswordsInput)
                RegularExpressions[keepreg] = -1
    return False



# Main  initializes structures and calls SolveCrossword
...
Crosswords={}
CrosswordsDictionary(csvfile,Crosswords)

NumberOfWords=len(Crosswords)
IntersectionsMatrix= [[-1 for i in range(NumberOfWords)] for j in range(NumberOfWords)]
Intersections(csvfile,IntersectionsMatrix)
RegularExpressions={}
RegularExpressionsDictionary(txtfile,RegularExpressions)
SolveCrossword(Crosswords)



solution = [2]*len(RegularExpressions)

for key in RegularExpressions:
    solution[RegularExpressions[key]] = key


# Prints results as follows word-id , regular expression used , word it produced
for key in sorted(Crosswords):
    reg=""
    word=Crosswords[key]
    completedword=''
    for letter in word:
        completedword +=letter
    print(key,solution[key],completedword)
